...
if __name__ == '__main__':
    ...
    model = RNNRBM(learning_rate=1e-5, visible_dim=1, \
        hidden_dim=300, rnn_hidden_dim=300, num_epochs=100)
    print ("Start training")
    preds, y, mae = model.train(X_train, y_train)
    print ("Finish training")

    train_results = list(zip(y, preds))
    df = pd.DataFrame(data=train_results, columns=['Y', 'Predictions'])
    df.to_csv('train_results.csv', index=False, header=False)
